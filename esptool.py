...
try:
    ...
    print_overwrite("Pyserial is not installed for %s. Check the README for installation instructions." % (sys.executable))
    raise
...
try:
    ...
    print_overwrite("The installed version (%s) of pyserial appears to be too old for esptool.py (Python interpreter %s). "
...
def print_overwrite(message, last_line=False):
    ...
    If output is not a TTY (for example redirected a pipe), no overwriting happens and this function is the same as print_overwrite().
    """
...
class ESPLoader(object):
    ...
    def detect_chip(port=DEFAULT_PORT, baud=ESP_ROM_BAUD, connect_mode='default_reset', trace_enabled=False,
        ...
        try:
            print_overwrite('Detecting chip type...', end='')
            sys.stdout.flush()
            ...
            for cls in [ESP8266ROM, ESP32ROM, ESP32S2ROM, ESP32S3BETA2ROM]:
                if chip_magic_value == cls.CHIP_DETECT_MAGIC_VALUE:
                    ...
                    inst._post_connect()
                    print_overwrite(' %s' % inst.CHIP_NAME, end='')
                    return inst
    ...
    def trace(self, message, *format_args):
        if self._trace_enabled:
            ...
            print_overwrite(prefix + (message % format_args))
    ...
    def connect(self, mode='default_reset', attempts=DEFAULT_CONNECT_ATTEMPTS, detecting=False):
        ...
        print_overwrite('Connecting...', end='')
        sys.stdout.flush()
        last_error = None
        ...
        try:
            ...
            print_overwrite('')  # end 'Connecting...' line
                ...
                if chip_magic_value != self.CHIP_DETECT_MAGIC_VALUE:
                    ...
                    for cls in [ESP8266ROM, ESP32ROM, ESP32S2ROM, ESP32S3BETA2ROM]:
                        if chip_magic_value == cls.CHIP_DETECT_MAGIC_VALUE:
                            ...
                            break
                    if actually is None:
                        print_overwrite(("WARNING: This chip doesn't appear to be a %s (chip magic value 0x%08x). " +
    ...
    def flash_begin(self, size, offset, begin_rom_encrypted=False):
        ...
        if size != 0 and not self.IS_STUB:
            print_overwrite("Took %.2fs to erase flash block" % (time.time() - t))
        return num_blocks
    ...
    def run_stub(self, stub=None):
        ...
        # Upload
        print_overwrite("Uploading stub...")
        ...
        print_overwrite("Running stub...")
        self.mem_finish(stub['entry'])

        p = self.read()
        if p != b'OHAI':
            raise FatalError("Failed to start stub. Unexpected response: %s" % p)
        print_overwrite("Stub running...")
    ...
    def flash_defl_begin(self, size, compsize, offset):
        ...
        print_overwrite("Compressed %d bytes to %d..." % (size, compsize))
    ...
    def get_crystal_freq(self):
        ...
        if abs(norm_xtal - est_xtal) > 1:
            print_overwrite("WARNING: Detected crystal freq %.2fMHz is quite different to normalized freq %dMHz. Unsupported crystal in use?" % (est_xtal, norm_xtal))
        return norm_xtal
...
class ESP32ROM(ESPLoader):
    ...
    def override_vddsdio(self, new_voltage):
        ...
        print_overwrite("VDDSDIO regulator set to %s" % new_voltage)
...
class ESP32S2ROM(ESP32ROM):
    ...
    def _check_if_can_reset(self):
        ...
        if os.getenv("ESPTOOL_TESTING") is not None:
            print_overwrite("ESPTOOL_TESTING is set, ignoring strapping mode check")
        ...
        if strap_reg & self.GPIO_STRAP_SPI_BOOT_MASK == 0 and force_dl_reg & self.RTC_CNTL_FORCE_DOWNLOAD_BOOT_MASK == 0:
            print_overwrite("ERROR: {} chip was placed into download mode using GPIO0.\n"
    ...
    def warn_if_unusual_segment(self, offset, size, is_irom_segment):
        if not is_irom_segment:
            if offset > 0x40200000 or offset < 0x3ffe0000 or size > 65536:
                print_overwrite('WARNING: Suspicious segment 0x%x, length %d' % (offset, size))
    ...
    def __init__(self, load_file=None):
        ...
        if load_file is not None:
            ...
            if segments != ESPBOOTLOADER.IMAGE_V2_SEGMENT:
                ...
                print_overwrite('Warning: V2 header has unexpected "segment" count %d (usually 4)' % segments)
            ...
            if first_flash_mode != self.flash_mode:
                print_overwrite('WARNING: Flash mode value in first header (0x%02x) disagrees with second (0x%02x). Using second value.'
                      % (first_flash_mode, self.flash_mode))
            if first_flash_size_freq != self.flash_size_freq:
                print_overwrite('WARNING: Flash size/freq value in first header (0x%02x) disagrees with second (0x%02x). Using second value.'
                      % (first_flash_size_freq, self.flash_size_freq))
            if first_entrypoint != self.entrypoint:
                print_overwrite('WARNING: Entrypoint address in first header (0x%08x) disagrees with second header (0x%08x). Using second value.'
                      % (first_entrypoint, self.entrypoint))
...
class ESP32FirmwareImage(BaseFirmwareImage):
    ...
    def load_extended_header(self, load_file):
        ...
        if chip_id != self.ROM_LOADER.IMAGE_CHIP_ID:
            print_overwrite(("Unexpected chip id in image. Expected %d but value was %d. " +
        ...
        if any(f for f in fields[6:-1] if f != 0):
            print_overwrite("Warning: some reserved header fields have non-zero values. This image may be from a newer esptool.py?")
...
class ELFFile(object):
    ...
    def _read_sections(self, f, section_header_offs, section_header_count, shstrndx):
        ...
        if sec_type != ELFFile.SEC_TYPE_STRTAB:
            print_overwrite('WARNING: ELF file has incorrect STRTAB section type 0x%02x' % sec_type)
        f.seek(sec_offs)
...
def load_ram(esp, args):
    ...
    print_overwrite('RAM boot...')
    for seg in image.segments:
        size = len(seg.data)
        print_overwrite('Downloading %d bytes at %08x...' % (size, seg.addr), end=' ')
        sys.stdout.flush()
        ...
        while len(seg.data) > 0:
            ...
            seq += 1
        print_overwrite('done!')

    print_overwrite('All segments done, executing at %08x' % image.entrypoint)
    esp.mem_finish(image.entrypoint)


def read_mem(esp, args):
    print_overwrite('0x%08x = 0x%08x' % (args.address, esp.read_reg(args.address)))


def write_mem(esp, args):
    esp.write_reg(args.address, args.value, args.mask, 0)
    print_overwrite('Wrote %08x, mask %08x to %08x' % (args.value, args.mask, args.address))
...
def detect_flash_size(esp, args):
    if args.flash_size == 'detect':
        ...
        if args.flash_size is None:
            print_overwrite('Warning: Could not auto-detect Flash size (FlashID=0x%x, SizeID=0x%x), defaulting to 4MB' % (flash_id, size_id))
            args.flash_size = '4MB'
        else:
            print_overwrite('Auto-detected Flash size:', args.flash_size)
...
def _update_image_flash_params(esp, address, args, image):
    ...
    if magic != esp.ESP_IMAGE_MAGIC:
        print_overwrite("Warning: Image file at 0x%x doesn't look like an image file, so not changing any flash settings." % address)
        return image
    ...
    try:
        ...
        print_overwrite("Warning: Image file at 0x%x is not a valid %s image, so not changing any flash settings." % (address,esp.CHIP_NAME))
        return image
    ...
    if flash_params != image[2:4]:
        print_overwrite('Flash params set to 0x%04x' % struct.unpack(">H", flash_params))
...
def write_flash(esp, args):
    ...
    if args.encrypt:
        ...
        if not esp.secure_download_mode:
            ...
            if crypt_cfg_efuse is not None and crypt_cfg_efuse != 0xF:
                print_overwrite('Unexpected FLASH_CRYPT_CONFIG value: 0x%x' % (crypt_cfg_efuse))
                do_write = False

            enc_key_valid = esp.is_flash_encryption_key_valid()

            if not enc_key_valid:
                print_overwrite('Flash encryption key is not programmed')
                do_write = False

        for address, argfile in args.addr_filename:
            if address % esp.FLASH_ENCRYPTED_WRITE_ALIGN:
                print_overwrite("File %s address 0x%x is not %d byte aligned, can't flash encrypted" %
    ...
    if args.encrypt and args.compress:
        ...
        print_overwrite('Will flash uncompressed')
        args.compress = False

    for address, argfile in args.addr_filename:
        if args.no_stub:
            print_overwrite('Erasing flash...')
        image = pad_to(argfile.read(), esp.FLASH_ENCRYPTED_WRITE_ALIGN if args.encrypt else 4)
        if len(image) == 0:
            print_overwrite('WARNING: File %s is empty' % argfile.name)
            continue
        ...
        if not args.encrypt and not esp.secure_download_mode:
            try:
                ...
                if res != calcmd5:
                    print_overwrite('File  md5: %s' % calcmd5)
                    print_overwrite('Flash md5: %s' % res)
                    print_overwrite('MD5 of 0xFF is %s' % (hashlib.md5(b'\xFF' * uncsize).hexdigest()))
                    raise FatalError("MD5 of file does not match data in flash!")
                else:
                    print_overwrite('Hash of data verified.')
    ...
    if args.verify:
        print_overwrite('Verifying just-written flash...')
        print_overwrite('(This option is deprecated, flash contents are now always read back after flashing.)')
        verify_flash(esp, args)


def image_info(args):
    image = LoadFirmwareImage(args.chip, args.filename)
    print_overwrite('Image version: %d' % image.version)
    print_overwrite('Entry point: %08x' % image.entrypoint if image.entrypoint != 0 else 'Entry point not set')
    print_overwrite('%d segments' % len(image.segments))
    print_overwrite()
    idx = 0
    for seg in image.segments:
        idx += 1
        seg_name = ", ".join([seg_range[2] for seg_range in image.ROM_LOADER.MEMORY_MAP if seg_range[0] <= seg.addr < seg_range[1]])
        print_overwrite('Segment %d: %r [%s]' % (idx, seg, seg_name))
    calc_checksum = image.calculate_checksum()
    print_overwrite('Checksum: %02x (%s)' % (image.checksum,
    ...
    try:
        ...
        if image.append_digest:
            ...
            print_overwrite('Validation Hash: %s' % digest_msg)
    except AttributeError:
...
def elf2image(args):
    ...
    if args.chip == 'auto':  # Default to ESP8266 for backwards compatibility
        print_overwrite("Creating image for ESP8266...")
        args.chip = 'esp8266'
...
def read_mac(esp, args):
    ...
    def print_mac(label, mac):
        print_overwrite('%s: %s' % (label, ':'.join(map(lambda x: '%02x' % x, mac))))
    print_mac("MAC", mac)
...
def chip_id(esp, args):
    try:
        chipid = esp.chip_id()
        print_overwrite('Chip ID: 0x%08x' % chipid)
    except NotSupportedError:
        print_overwrite('Warning: %s has no Chip ID. Reading MAC instead.' % esp.CHIP_NAME)
        read_mac(esp, args)


def erase_flash(esp, args):
    print_overwrite('Erasing flash (this may take a while)...')
    t = time.time()
    esp.erase_flash()
    print_overwrite('Chip erase completed successfully in %.1fs' % (time.time() - t))


def erase_region(esp, args):
    print_overwrite('Erasing region (may be slow depending on size)...')
    t = time.time()
    esp.erase_region(args.address, args.size)
    print_overwrite('Erase completed successfully in %.1f seconds.' % (time.time() - t))
...
def flash_id(esp, args):
    flash_id = esp.flash_id()
    print_overwrite('Manufacturer: %02x' % (flash_id & 0xff))
    flid_lowbyte = (flash_id >> 16) & 0xFF
    print_overwrite('Device: %02x%02x' % ((flash_id >> 8) & 0xff, flid_lowbyte))
    print_overwrite('Detected flash size: %s' % (DETECTED_FLASH_SIZES.get(flid_lowbyte, "Unknown")))
...
def verify_flash(esp, args):
    ...
    for address, argfile in args.addr_filename:
        ...
        print_overwrite('Verifying 0x%x (%d) bytes @ 0x%08x in flash against %s...' % (image_size, image_size, address, argfile.name))
        # Try digest first, only read if there are differences.
        digest = esp.flash_md5sum(address, image_size)
        expected_digest = hashlib.md5(image).hexdigest()
        if digest == expected_digest:
            print_overwrite('-- verify OK (digest matched)')
            continue
        else:
            differences = True
            if getattr(args, 'diff', 'no') != 'yes':
                print_overwrite('-- verify FAILED (digest mismatch)')
                continue

        flash = esp.read_flash(address, image_size)
        assert flash != image
        diff = [i for i in range(image_size) if flash[i] != image[i]]
        print_overwrite('-- verify FAILED: %d differences, first @ 0x%08x' % (len(diff), address + diff[0]))
        for d in diff:
            flash_byte = flash[d]
            image_byte = image[d]
            if PYTHON2:
            ...
            print_overwrite('   %08x %02x %02x' % (address + d, flash_byte, image_byte))
    if differences:
        raise FatalError("Verify failed.")


def read_flash_status(esp, args):
    print_overwrite('Status value: 0x%04x' % esp.read_status(args.bytes))


def write_flash_status(esp, args):
    fmt = "0x%%0%dx" % (args.bytes * 2)
    args.value = args.value & ((1 << (args.bytes * 8)) - 1)
    print_overwrite(('Initial flash status: ' + fmt) % esp.read_status(args.bytes))
    print_overwrite(('Setting flash status: ' + fmt) % args.value)
    esp.write_status(args.value, args.bytes, args.non_volatile)
    print_overwrite(('After flash status:   ' + fmt) % esp.read_status(args.bytes))


def get_security_info(esp, args):
    (flags, flash_crypt_cnt, key_purposes) = esp.get_security_info()
    # TODO: better display
    print_overwrite('Flags: 0x%08x (%s)' % (flags, bin(flags)))
    print_overwrite('Flash_Crypt_Cnt: 0x%x' % flash_crypt_cnt)
    print_overwrite('Key_Purposes: %s' % (key_purposes,))


def version(args):
    print_overwrite(__version__)
...
def main(custom_commandline=None):
    ...
    print_overwrite('esptool.py v%s' % __version__)
    ...
    if operation_args[0] == 'esp':  # operation function takes an ESPLoader connection object
        ...
        if args.port is None:
            ...
            print_overwrite("Found %d serial ports" % len(ser_list))
        else:
            ser_list = [args.port]
        esp = None
        for each_port in reversed(ser_list):
            print_overwrite("Serial port %s" % each_port)
            try:
                if args.chip == 'auto':
                ...
                if args.port is not None:
                    raise
                print_overwrite("%s failed to connect: %s" % (each_port, err))
                esp = None
        if esp is None:
            raise FatalError("Could not connect to an Espressif device on any of the %d available serial ports." % len(ser_list))

        if esp.secure_download_mode:
            print_overwrite("Chip is %s in Secure Download Mode" % esp.CHIP_NAME)
        else:
            print_overwrite("Chip is %s" % (esp.get_chip_description()))
            print_overwrite("Features: %s" % ", ".join(esp.get_chip_features()))
            print_overwrite("Crystal is %dMHz" % esp.get_crystal_freq())
            read_mac(esp, args)

        if not args.no_stub:
            if esp.secure_download_mode:
                print_overwrite("WARNING: Stub loader is not supported in Secure Download Mode, setting --no-stub")
                args.no_stub = True
            else:
                esp = esp.run_stub()
        ...
        if args.baud > initial_baud:
            try:
                ...
                print_overwrite("WARNING: ROM doesn't support changing baud rate. Keeping initial baud rate %d" % initial_baud)

        # override common SPI flash parameter stuff if configured to do so
        if hasattr(args, "spi_connection") and args.spi_connection is not None:
            if esp.CHIP_NAME != "ESP32":
                raise FatalError("Chip %s does not support --spi-connection option." % esp.CHIP_NAME)
            print_overwrite("Configuring SPI flash mode...")
            esp.flash_spi_attach(args.spi_connection)
        elif args.no_stub:
            print_overwrite("Enabling default SPI flash mode...")
            # ROM loader doesn't enable flash unless we explicitly do it
            esp.flash_spi_attach(0)

        if hasattr(args, "flash_size"):
            print_overwrite("Configuring flash size...")
        ...
        if operation_func == load_ram:
            ...
            print_overwrite('Exiting immediately.')
        elif args.after == 'hard_reset':
            print_overwrite('Hard resetting via RTS pin...')
            esp.hard_reset()
        elif args.after == 'soft_reset':
            print_overwrite('Soft resetting...')
            # flash_finish will trigger a soft reset
            esp.soft_reset(False)
        else:
            print_overwrite('Staying in bootloader.')
            if esp.IS_STUB:
...
def expand_file_arguments():
    ...
    for arg in sys.argv:
        if arg.startswith("@"):
            ...
            new_args.append(arg)
    if expanded:
        print_overwrite("esptool.py %s" % (" ".join(new_args[1:])))
        sys.argv = new_args
...
class FlashSizeAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        try:
            ...
            print_overwrite("WARNING: Flash size arguments in megabits like '%s' are deprecated." % (values[0]))
            print_overwrite("Please use the equivalent size '%s'." % (value))
            print_overwrite("Megabit arguments may be removed in a future release.")
        except KeyError:
            value = values[0]
